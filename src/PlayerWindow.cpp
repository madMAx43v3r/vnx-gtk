...
PlayerWindow::PlayerWindow(const std::string& vnx_name_)
	:	PlayerWindowBase(vnx_name_),
...
void PlayerWindow::handle(std::shared_ptr<const vnx::PlayerStatus> value)
{
	player_status = value;
}

bool PlayerWindow::on_render()
{
	if(!player_status) {
...
void PlayerWindow::on_topics()
{
	// TODO
}

void PlayerWindow::on_info()
{
	info_display.show();
}
...
void PlayerWindow::on_seek_relative(double secs)
{
	player_sync.seek_by_time_async(secs * 1e6);
}

void PlayerWindow::on_seek_absolute()
{
